 ...
 * {@link JanusGraph}. Since JanusGraph is a transactional graph database, all
 * interactions with the graph are mitigated by a JanusGraphTransaction.
 * <p/>
...
public interface JanusGraphTransaction extends Transaction {
	...
	/**
	 * Creates a new vertex in the graph with the given vertex id and the given
	 * vertex label. Note, that an exception is thrown if the vertex id is not a
	 * valid JanusGraph vertex id or if a vertex with the given id already
	 * exists.
	 * <p/>
	 ...
	 *
	 * @param id
	 *            vertex id of the vertex to be created
	 * @param vertexLabel
	 *            vertex label for this vertex - can be null if no vertex label
	 *            should be set.
	 * @return New vertex
	 */
	 ...
	 * {@link JanusGraphTransaction#addVertex(Long, VertexLabel)} in case
	 * starVertex not provided. Otherwise pass starVertex to
	 * {@link IDPlacementStrategy} for better graph partitioning
	 * 
	 * @param id
	 * @param label
	 * @param starVertex
	 ...
	 * {@link org.apache.tinkerpop.gremlin.structure.Graph#vertices(Object...)}
	 * instead.
	 *
	 * @param id
	 ...
	 * <p/>
	 * Will attempt to persist all modifications which may result in exceptions
	 * in case of persistence failures or lock contention. <br />
	 * The call releases data structures if possible. All element references
	 * (e.g. vertex objects) retrieved through this transaction are stale after
	 * the transaction closes and should no longer be used.
	 *
	...
	/**
	 * Checks whether any changes to the graph database have been made in this
	 * transaction.
	 * <p/>
