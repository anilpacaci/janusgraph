...
public class StandardJanusGraphTx extends JanusGraphBlueprintsTransaction
	...
	/**
	 * This is a workaround for #893. Cache sizes small relative to the level of
	 * thread parallelism can lead to JanusGraph generating multiple copies of a
	 * single vertex in a single transaction.
	 */
	...
	// ####### Vertex Cache
	/**
	 * Keeps track of vertices already loaded in memory. Cannot release vertices
	 * with added relations.
	 */
	private final VertexCache vertexCache;

	// ######## Data structures that keep track of new and deleted elements
	// These data structures cannot release elements, since we would loose track
	...
	// ######## Index Caches
	/**
	 * Caches the result of index calls so that repeated index queries don't
	 * need to be passed to the IndexProvider. This cache will drop entries when
	 * it overflows since the result set can always be retrieved from the
	 * IndexProvider
	 */
	private final Cache<JointIndexQuery.Subquery, List<Object>> indexCache;
	/**
	 * Builds an inverted index for newly added properties so they can be
	 * considered in index queries. This cache my not release elements since
	 * that would entail an expensive linear scan over addedRelations
	 */
	private final IndexCache newVertexIndexEntries;

	// ######## Lock applications
	/**
	 * Transaction-local data structure for unique lock applications so that
	 * conflicting applications can be discovered at the transactional level.
	 */
	private ConcurrentMap<LockTuple, TransactionLock> uniqueLocks;

	// ####### Other Data structures
	/**
	 * Caches JanusGraph types by name so that they can be quickly retrieved
	 * once they are loaded in the transaction. Since type retrieval by name is
	 * common and there are only a few types, since cache is a simple map (i.e.
	 * no release)
	 */
	private final Map<String, Long> newTypeCache;

	/**
	 * Used to assign temporary ids to new vertices and relations added in this
	 * transaction. If ids are assigned immediately, this is not used. This
	 * IDPool is shared across all elements.
	 */
	...
	public StandardJanusGraphTx(StandardJanusGraph graph, TransactionConfiguration config) {
		...
		if (null != config.getGroupName()) {
			...
			elementProcessor = new MetricsQueryExecutor<GraphCentricQuery, JanusGraphElement, JointIndexQuery>(
					config.getGroupName(), "graph", elementProcessorImpl);
			edgeProcessor = new MetricsQueryExecutor<VertexCentricQuery, JanusGraphRelation, SliceQuery>(
	...
	private InternalVertex getExistingVertex(long vertexid) {
		// return vertex no matter what, even if deleted, and assume the id has
		// the correct format
	...
	private class VertexConstructor implements Retriever<Long, InternalVertex> {
		...
		public InternalVertex get(Long vertexid) {
			...
			long canonicalVertexId = idInspector.isPartitionedVertex(vertexid)
					? idManager.getCanonicalVertexId(vertexid) : vertexid;
			if (verifyExistence) {
	...
	private Iterable<InternalVertex> getInternalVertices() {
		...
		if (!addedRelations.isEmpty()) {
			...
			while (viter.hasNext()) {
				if (viter.next() instanceof JanusGraphSchemaElement)
					viter.remove();
			}
	...
	private TransactionLock getLock(final LockTuple la) {
		...
		// TODO: clean out no longer used locks from uniqueLocks when it grows
		// to large (use ReadWriteLock to protect against race conditions)
	...
	private TransactionLock getUniquenessLock(final JanusGraphVertex out, final InternalRelationType type,
			final Object in) {
	...
	public JanusGraphVertexProperty addProperty(VertexProperty.Cardinality cardi, JanusGraphVertex vertex,
		...
		uniqueLock.lock(LOCK_TIMEOUT);
		try {
			// //Check vertex-centric uniqueness -> this doesn't really make
			// sense to check
			// if (config.hasVerifyUniqueness()) {
			// if (cardinality == Cardinality.SINGLE) {
			// if (!Iterables.isEmpty(query(vertex).type(key).properties()))
			// throw new SchemaViolationException("A property with the given key
			// [%s] already exists on the vertex [%s] and the property key is
			// defined as single-valued", key.name(), vertex);
			// }
			// if (cardinality == Cardinality.SET) {
			// if
			// (!Iterables.isEmpty(Iterables.filter(query(vertex).type(key).properties(),
			// new Predicate<JanusGraphVertexProperty>() {
			// @Override
			// public boolean apply(@Nullable JanusGraphVertexProperty
			// janusgraphProperty) {
			// return normalizedValue.equals(janusgraphProperty.value());
			// }
			// })))
			// throw new SchemaViolationException("A property with the given key
			// [%s] and value [%s] already exists on the vertex and the property
			// key is defined as set-valued", key.name(), normalizedValue);
			// }
			// }
			...
			if (cardi == VertexProperty.Cardinality.single || cardi == VertexProperty.Cardinality.set) {
				...
				/*
				 * If we are simply overwriting a vertex property, then we don't
				 * have to explicitly remove it thereby saving a read operation
				 * However, this only applies if 1) we don't lock on the
				 * property key or consistency checks are disabled and 2) there
				 * are no indexes for this property key 3) the cardinalities
				 * match (if we overwrite a set with single, we need to read all
				...
				if ((!config.hasVerifyUniqueness()
					...
					// Only delete in-memory so as to not trigger a read from
					// the database which isn't necessary because we will
					// overwrite blindly
			...
			StandardVertexProperty prop = new StandardVertexProperty(
					IDManager.getTemporaryRelationID(temporaryIds.nextID()), key, (InternalVertex) vertex,
					normalizedValue, ElementLifeCycle.New);
			if (config.hasAssignIDsImmediately())
				graph.assignID(prop);
	...
	public final JanusGraphSchemaVertex makeSchemaVertex(JanusGraphSchemaCategory schemaCategory, String name,
		...
		if (schemaCategory.isRelationType()) {
			...
			schemaVertex = new JanusGraphSchemaVertex(this,
					IDManager.getTemporaryVertexID(IDManager.VertexIDType.GenericSchemaType, temporaryIds.nextID()),
					ElementLifeCycle.New);
		}
	...
	// this is critical path we can't allow anything heavier then assertion in
	// here
	@Override
	...
	public void executeMultiQuery(final Collection<InternalVertex> vertices, final SliceQuery sq,
		...
		if (!vids.isEmpty()) {
			List<EntryList> results = QueryProfiler.profile(profiler, sq, true,
					q -> graph.edgeMultiQuery(vids, q, txHandle));
			int pos = 0;
			for (JanusGraphVertex v : vertices) {
	...
	public final QueryExecutor<VertexCentricQuery, JanusGraphRelation, SliceQuery> edgeProcessorImpl = new QueryExecutor<VertexCentricQuery, JanusGraphRelation, SliceQuery>() {
		...
		public Iterator<JanusGraphRelation> getNew(final VertexCentricQuery query) {
			...
			if (vertex.isNew() || vertex.hasAddedRelations()) {
				return (Iterator) vertex.getAddedRelations(new Predicate<InternalRelation>() {
					// Need to filter out self-loops if query only asks for one
					// direction
		...
		public boolean hasDeletions(VertexCentricQuery query) {
			...
			if (vertex.isNew())
				return false;
			// In addition to deleted, we need to also check for added relations
			// since those can potentially
		...
		public boolean isDeleted(VertexCentricQuery query, JanusGraphRelation result) {
			if (deletedRelations.containsKey(result.longId()) || result != ((InternalRelation) result).it())
				return true;
			// Check if this relation is replaced by an added one due to a
		...
		@Override
		public Iterator<JanusGraphRelation> execute(final VertexCentricQuery query, final SliceQuery sq,
	...
	public final QueryExecutor<GraphCentricQuery, JanusGraphElement, JointIndexQuery> elementProcessorImpl = new QueryExecutor<GraphCentricQuery, JanusGraphElement, JointIndexQuery>() {
		...
		private PredicateCondition<PropertyKey, JanusGraphElement> getEqualityCondition(
		...
		public Iterator<JanusGraphElement> getNew(final GraphCentricQuery query) {
			// If the query is unconstrained then we don't need to add new
			// elements, so will be picked up by getVertices()/getEdges() below
			...
			if (query.getResultType() == ElementCategory.VERTEX && hasModifications()) {
				...
				PredicateCondition<PropertyKey, JanusGraphElement> standardIndexKey = getEqualityCondition(
						query.getCondition());
				...
				if (standardIndexKey == null) {
					...
					ConditionUtil.traversal(query.getCondition(), new Predicate<Condition<JanusGraphElement>>() {
						...
						public boolean apply(@Nullable Condition<JanusGraphElement> cond) {
							Preconditions.checkArgument(
									cond.getType() != Condition.Type.LITERAL || cond instanceof PredicateCondition);
					...
					vertices = com.google.common.collect.Iterators.transform(newVertexIndexEntries
							...
							new Function<JanusGraphVertexProperty, JanusGraphVertex>() {
								@Nullable
								@Override
				...
				return (Iterator) com.google.common.collect.Iterators.filter(vertices,
						new Predicate<JanusGraphVertex>() {
							@Override
		...
		public boolean isDeleted(GraphCentricQuery query, JanusGraphElement result) {
			if (result == null || result.isRemoved())
				...
				// Loaded relations are immutable so we don't need to check
				// those
				// New relations could be modified in this transaction to now
				// longer match the query, hence we need to
